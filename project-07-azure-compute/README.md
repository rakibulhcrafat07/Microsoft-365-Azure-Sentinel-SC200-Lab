...
- [Project 7 — Azure Compute: Virtual Machines, AD DS & Defender for Identity](#project-7--azure-compute-virtual-machines-ad-ds--defender-for-identity-az-104--sc-200)
...
## Project 7 — Azure Compute: Virtual Machines, AD DS & Defender for Identity (AZ-104 / SC-200)
...
**📄 Full detail (143 screenshots, every sub-step documented): [`project-07-azure-compute/README.md`](./project-07-azure-compute/README.md)**

Almost nothing in this project worked on the first try — a VM size that doesn't exist in this region, an NSG that silently blocked RDP, Bastion blocked for a completely different reason than the RDP client was, a resize that hit a shared regional quota, a Portal quota checker that flatly disagreed with the real Quotas page, a Defender for Identity workspace that refused to provision the first time, and a storage account correctly refusing a public download mid-sensor-install. None of it was hidden — the dead ends are the part worth reading. AWS EC2 was left out on purpose; the lab stays Azure/Microsoft-only.

Summary of what was built:

- **Windows Server 2022 VM** (`vm-win-addc`) and **Ubuntu VM** (`vm-ubuntu-lab`), both joined to the existing `vnet-lab-sc200`/`subnet-app` from Project 6 — reachable only after three separate rounds of access troubleshooting (a missing RDP rule on the inherited NSG, a local ISP blocking outbound RDP entirely, and Bastion needing its own separate NSG rule since its traffic comes from `AzureBastionSubnet`, not "My IP")
- **VM resize** — `D2s_v7` → `D2ds_v7` on the Linux VM, after hitting a genuine regional vCPU quota block and discovering the Portal's quota check can lag behind the real Quotas page
- **Managed disk + snapshot** — a 32 GiB data disk attached to `vm-win-addc`, initialized as `D:` (the future home of the AD database), with an incremental snapshot taken as a backup point
- **Custom Script Extension** — installed via a script uploaded to the Project 5 storage account (`strakibullab01`/`mycontainer`), verified by a timestamped proof file written inside the VM itself, not just a green checkmark in the Portal
- **Availability Zone** — attempted on a dedicated demo VM, blocked by a genuine mismatch between the Create wizard's quota check and the real Quotas page; documented honestly as a Portal limitation rather than forced around
- **AD DS promotion** — `vm-win-addc` promoted to the first Domain Controller of a brand-new forest, `rakibul.local`, confirmed via the NTDS and Kerberos KDC services both showing Running
- **Defender for Identity sensor** — installed on the new domain controller after several wrong turns finding the right Defender portal page and a first-attempt workspace provisioning failure; the classic installer was used (the newer "Activate servers" method silently needs Defender for Endpoint device onboarding), with the installer file itself moved onto the VM via a time-limited SAS token after a direct public download was correctly blocked
- **VM shutdown** — both VMs stopped (deallocated) at the end of the project to release compute quota and stop billing, without losing any configuration

![Defender for Identity sensor registered against the real rakibul.local domain controller](./project-07-azure-compute/images/140_sensor_registered_not_healthy.png)
*Type: Domain controller, Domain: rakibul.local — the sensor now reporting real on-prem-style identity telemetry, exactly what later SC-200 detection projects on the roadmap will need.*
...
17. **A VM inherits its subnet's NSG.** `vm-win-addc` landed in the same `subnet-app` used since Project 6, which already had an SSH-only NSG attached — RDP had to be explicitly added, it wasn't assumed open just because the VM was new.
18. **Bastion traffic originates from its own dedicated subnet, not the user's IP.** An NSG rule scoped to "My IP" doesn't cover Bastion automatically — a second, separate rule for the `AzureBastionSubnet` range was needed before the browser-based connection worked.
19. **Regional vCPU quota is shared across every VM in the same size family**, and the Azure Portal's live quota check can visibly lag behind the authoritative Quotas page — when the two disagree, the Quotas page is the one to trust.
20. **Installing a server role and promoting that server are two distinct steps.** Adding the AD DS role through Server Manager does nothing on its own until the separate promotion wizard runs and the server reboots into an actual domain controller.
21. **A Defender for Identity workspace provisions lazily, on first access** — not the instant a license is assigned — and its newer "Activate servers" sensor method silently depends on Defender for Endpoint device inventory that a domain controller may never have been onboarded to.
22. **Secure-by-default storage blocks public blob access even for purely internal, same-tenant transfers.** Moving the MDI sensor installer onto the VM needed a short-lived SAS token, not a relaxation of the storage account's own security.
23. **Exchange mail flow rules are disabled by default after creation** and must be manually enabled before they will actually process anything.
24. **Anti-phishing, anti-spam, and anti-malware are three genuinely separate detection layers**, each with its own action pipeline (reject, quarantine, move to junk) — full mail protection needs all three configured, not just the tenant defaults.
25. **The EICAR test string is completely harmless and industry-standard**, but it still has to be delivered as a real file — some browsers will render a `.com`/`.txt` test file as plain text instead of downloading it, which looks like a failure but isn't one.
26. **Wizard-level validation errors don't always explain themselves.** A generic "Client Error" on the anti-phishing policy wizard turned out to trace back to one specific unset dropdown (the quarantine policy) several steps back in the flow.
27. **Attack Simulation Training's full lifecycle — delivery, click, credential harvest, education, and reporting — can be observed end to end** using only the lab's existing test accounts, at zero additional cost.
